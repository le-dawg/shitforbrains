...
$dato = date('d-m-Y);
$result = mysql_query("SELECT * FROM proservice_rapport where ans=20 AND dato=$dato");
...
$bro = $row['bro2'];
$status = $row['bro'];
$result = mysql_query("SELECT * FROM promed where id=20");
   while($row = mysql_fetch_array($result)) {
$navn = $row['navn'];
...
$result5 = mysql_query("SELECT * FROM promed WHERE id = $id");
...
<td width="7%" height="20px" bgcolor="#eeeeee"><font size="10px"> <b>Status</b></font></td>
 ...
 <?php          
            $result  = mysql_query("SELECT * FROM proservice_rapport WHERE ans = '$ans' AND dato = '$test' AND timer !='-' ORDER BY id ASC ");
	    ...
	    if (is_numeric($service_raport)) {
            $result2 = mysql_query("SELECT * FROM proservice WHERE id = '$service_raport'");
...
$mail->AddAddress("office@proconsultwebshop.dk");
...
{
   header("Location:index.php?side=dagseddel&sendt=ja");
} //`the end`


$dato = date('Y-m-d H:i:s' , strtotime("$dato"));
mysql_query("UPDATE `promed` SET `sidste_dagsseld`='$dato' WHERE id='$id'");
